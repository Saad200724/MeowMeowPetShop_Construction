...
import { Dog, Search, Grid3X3, List } from 'lucide-react';
...
export default function DogBestSeller() {
  ...
  const dogBestsellerProducts = (allProducts as any[]).filter((product: any) => 
    product.isBestseller && dogCategories.includes(product.category)
  );

  // Filter products based on search query
  const filteredProducts = dogBestsellerProducts.filter((product: any) =>
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4">
            <h2 className="text-2xl font-bold">
              All Dog Bestsellers {!isLoading && `(${filteredProducts.length})`}
            </h2>
          ...
          {isLoading ? (
            <div className="text-center py-12">
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 mb-4">
                {[...Array(8)].map((_, i) => (
                  <div key={i} className="bg-white rounded-lg shadow-md h-80 animate-pulse">
                    <div className="bg-blue-200 h-48 rounded-t-lg"></div>
              ...
              <p className="text-lg text-blue-600 font-medium animate-pulse">Loading dog bestsellers...</p>
            </div>
          ) : filteredProducts.length === 0 ? (
            <Card className="p-8">
              <div className="text-center">
                <Dog className="h-16 w-16 text-gray-300 mx-auto mb-4" />
                <h3 className="text-xl font-semibold text-gray-600 mb-2">No Dog Bestsellers Found</h3>
                <p className="text-gray-500">
                  {searchQuery ? 'Try adjusting your search terms' : 'No dog bestseller products available at the moment.'}
                </p>
              </div>
            </Card>
          ) : (
