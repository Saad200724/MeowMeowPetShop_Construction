...
export default function ProductCard({ product }: ProductCardProps) {
  ...
  return (
    <Link href={`/product/${product.id?.toString() ?? product._id}`} data-testid={`product-link-${product.id?.toString() ?? product._id}`}>
      <div className="bg-white rounded-2xl shadow-sm hover:shadow-lg transition-all duration-300 hover:-translate-y-1 relative overflow-hidden group border border-gray-100 flex flex-col w-[160px] h-[260px] cursor-pointer">
      {/* Discount Badge */}
      {product.discount && (
