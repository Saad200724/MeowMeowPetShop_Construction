...
export default function ProductCard({ product }: ProductCardProps) {
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm hover:shadow-lg transition-all duration-300 hover:-translate-y-1 relative overflow-hidden group border border-gray-100 h-auto flex flex-col w-[300px] sm:w-[280px] md:w-[250px] max-w-[300px] sm:max-w-[280px] md:max-w-[250px]">
      {/* Discount Badge */}
      {product.discount && (
      ...
      <div className="relative overflow-hidden bg-white rounded-t-2xl h-56 sm:h-48 md:h-40 flex items-center justify-center flex-shrink-0">
        <img 
          src={product.image} 
          alt={product.name} 
      ...
      <div className="p-4 flex flex-col justify-between space-y-3 flex-1">
        ...
        <h4 className="font-semibold text-lg sm:text-base md:text-sm text-gray-900 group-hover:text-[#26732d] transition-colors line-clamp-2 leading-tight text-left">
          {product.name}
        </h4>
        ...
        <div className="space-y-2 flex-1 flex flex-col justify-between">
          <div className="text-left">
            <div className="flex items-center gap-2 mb-1">
              <span className="text-xl sm:text-lg md:text-lg font-bold text-[#26732d]">
                ৳{typeof product.price === 'string' ? product.price : product.price.toLocaleString()}
              </span>
              {oldPriceValue && (
                <span className="text-base sm:text-sm text-gray-500 line-through">
          ...
          <Button 
            variant="outline"
            size="sm"
            className="w-full rounded-full border-2 border-gray-200 text-gray-700 hover:border-[#26732d] hover:text-[#26732d] hover:bg-[#26732d]/5 transition-all duration-200 h-12 sm:h-10 md:h-9 text-base sm:text-sm font-medium"
